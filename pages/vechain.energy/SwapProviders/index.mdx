...
A Swap Provider provides access to a Token-Swap within a dApp and a Fiat-On-Ramp to onboard new users into the ecosystem.
...
For Fiat purchases the user needs to be redirect to a Payment-Processor that ensures legal compliance, but which still adds friction but removes the need to search for a trustworthy way to onboard the ecosystem.
